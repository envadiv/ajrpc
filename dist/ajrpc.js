...
(function (JsonRpcErrorCode) {
    ...
    JsonRpcErrorCode[JsonRpcErrorCode["Closed"] = 10] = "Closed";
    JsonRpcErrorCode[JsonRpcErrorCode["HandlerError"] = 20] = "HandlerError";
...
class JsonRpcError extends Error {
     ...
     * as a string.
     */
    constructor(code, message, id, data) {
        super(message);
        this.code = code;
        this.id = id;
        this.data = data;
...
exports.PairedChannel = PairedChannel;
/**
...
class HeartbeatChannel {
     ...
     */
    constructor(channel, timeout, callback) {
...
exports.HeartbeatChannel = HeartbeatChannel;
/**
...
class JsonRpc {
    ...
    close(code, reason) {
        ...
        for (let resolution of this.pending.values()) {
            resolution.reject(new JsonRpcError(JsonRpcErrorCode.Closed, 'JSON-RPC closed before response received', `${resolution.method} ${JSON.stringify(resolution.params)}`));
        }
    }
    /**
    ...
    call(method, ...params) {
        ...
        if (this.inBatch) {
            (_a = this.batchMessages) === null || _a === void 0 ? void 0 : _a.push(message);
        }
        else {
    ...
    batch(fn) {
        ...
        try {
            ...
            if (this.batchMessages.length > 0) {
                ...
                throw new JsonRpcError(JsonRpcErrorCode.InternalError, "Empty batch", null);
            }
        }
        finally {
    ...
    incoming(message) {
        ...
        try {
            ...
            this.error(new JsonRpcError(JsonRpcErrorCode.InvalidRequest, error.message, null));
            return;
        }
            ...
            if ("method" in item) {
                ...
                if (id != null) {
                    if (this.methods.has(method)) {
                        const impl = this.methods.get(method);
                        let result;
                        try {
                            if (Array.isArray(params)) {
                                result = yield impl(...params);
                            }
                                ...
                                this.error(new JsonRpcError(JsonRpcErrorCode.InvalidParams, "The call parameters must be either a positional array or an object", id, JSON.stringify(item)));
                                return;
                            }
                        }
                        catch (error) {
                            this.error(new JsonRpcError(JsonRpcErrorCode.InternalError, error.message, id));
                            return;
                        }
                        ...
                        this.error(new JsonRpcError(JsonRpcErrorCode.MethodNotFound, `No available method named "${method}"`, id));
                    }
                }
                    ...
                    if (this.methods.has(method)) {
                        ...
                        try {
                            if (Array.isArray(params)) {
                                ...
                                this.error(new JsonRpcError(JsonRpcErrorCode.InvalidParams, "The call parameters must be either a positional array or an object", null, JSON.stringify(item)));
                            }
                        }
                        catch (error) {
                            this.error(error);
                        ...
                        this.error(new JsonRpcError(JsonRpcErrorCode.MethodNotFound, `No available method named "${method}"`, null));
                    }
                }
            }
            // handle results
                ...
                if (id != null) {
                    if (this.pending.has(String(id))) {
                        const resolution = this.pending.get(id);
                        resolution.resolve(result);
                        this.pending.delete(id);
                    }
                    else {
                        this.error(new JsonRpcError(JsonRpcErrorCode.InvalidRequest, `Unmatched id "${id}" in incoming result response`, null, JSON.stringify(item)));
                    }
                }
                else {
                    this.error(new JsonRpcError(JsonRpcErrorCode.InvalidRequest, 'No id field in result', null, JSON.stringify(item)));
                }
            }
            // handle errors
            else if ("error" in item) {
                const { message, code, id, data } = item["error"];
                if (id) {
                    if (this.pending.has(id)) {
                        const resolution = this.pending.get(id);
                        resolution.reject(new JsonRpcError(code, message, id, data));
                        this.pending.delete(id);
                    }
                    else {
                        ...
                        console.error(`Unmatched id "${id}" in incoming error response`, item);
                    }
                }
                else {
                    console.error("Unidentified error on JsonRpc peer:", message, code, data);
                }
            }
            else {
                ...
                this.error(new JsonRpcError(JsonRpcErrorCode.InvalidRequest, "The request object needs to have a method, result, or error. Original in data member.", "id" in item ? item["id"] : null, data));
            }
        });
    }
    /**
     ...
     */
    error(error) {
        console.log('JsonRpc.error() logging error before sending to peer:', error);
